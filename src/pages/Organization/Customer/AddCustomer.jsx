...
const [isEditMode, setIsEditMode] = useState(false);
   ...
   const handleGetCurrentLocation = () => {
    ...
    const REQUIRED_ACCURACY = isFirefox ? 20 : 2;

    notification.info({
        message: 'Getting Location',
        description: isFirefox
            ? 'Using Firefox location (may be less accurate)'
            : 'Waiting for high-accuracy GPS lock…',
        duration: 4,
    });

    const options = {
        enableHighAccuracy: !isFirefox,
        timeout: isFirefox ? 15000 : 30000,
        maximumAge: 0,
    };

    // 🔹 FIREFOX: Use getCurrentPosition
    if (isFirefox) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                notification.warning({
                    message: 'Limited Accuracy (Firefox)',
            ...
            (error) => {
                notification.error({
                    ...
                    description: 'Firefox could not determine location.',
                });
            },
            options
        );
    ...
    // 🔹 CHROME / OTHERS: Use watchPosition
    const watchId = navigator.geolocation.watchPosition(
        ...
        (error) => {
            notification.error({
                message: 'GPS Error',
                description: 'Unable to get accurate location.',
            });
            navigator.geolocation.clearWatch(watchId);
        },
        options
